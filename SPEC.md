...
| D4 | Scope | v1 ✅ + v2 ✅ + v3 OCSP ✅ (2026-07-17): Name/AlgId/OIDs/SPKI/TBS + core extensions, self-signed CA + issue cert, PKCS#10 CSR intake (PoP verified in Decode), CRL building, OCSP (request decode incl. nonce, signed BasicOCSPResponse builder, error responses). Still open from v3: chain helpers. Note: LibreSSL 3.3.6's `ocsp` client fails nonce checks even against its own responder — nonce echo is asserted byte-exactly in our tests instead. |
